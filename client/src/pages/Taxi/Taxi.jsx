 ...
 * 1. Functional Components: This is a modern React functional component that returns JSX.
 * 2. useState Hook: 
 *    - Used to manage the local state of the Estimate Inquiry Form (`formData`).
 *    - `useState` returns an array with the current state value and a function to update it.
 * 3. Controlled Inputs:
 *    - The inputs have `value={formData.fieldName}` and `onChange={handleInputChange}`.
 *    - This makes them "controlled components" because React holds the single source of truth for the inputs' values.
 * 4. Dynamic Event Handling:
 *    - `handleInputChange` uses ES6 computed property names `[id]: value` to update the specific state key dynamically.
 * 5. External Utility Integration:
 *    - Uses `generateBookingMessage` and `getWhatsAppLink` to format inputs and trigger a WhatsApp redirect.
 */
export const Taxi = () => {
  // Initialize state with default empty values for form inputs
  ...
  // Handle updates for all inputs dynamically based on their element ID
  const handleInputChange = (e) => {
    const { id, value } = e.target;
    setFormData((prev) => ({
      ...prev,
      [id]: value, // ES6 Computed Property Name updates the state dynamically
    }));
  };

  // Construct and send the inquiry message directly to WhatsApp on submission
  const handleEstimateRequest = (e) => {
    ...
    // Open WhatsApp link in a new browser tab
    window.open(getWhatsAppLink(message), '_blank');
  };

  return (
    <div className="bg-bg-primary min-h-screen">
      {/* SubHero Banner for Taxi Fleet */}
      <SubHero title="Our Premium Fleet & Tariffs" breadcrumb="Our Taxis" />

      {/* Main Container */}
      <main className="max-w-[1400px] mx-auto px-6 md:px-12 py-16 space-y-24">
        
        {/* FLEET SECTION */}
        <section id="taxi" className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {fleetData.map((car, index) => (
              <ScrollReveal key={car.id} direction="up" delay={index * 100} duration={600}>
                <div 
                  className={`bg-bg-secondary rounded-3xl p-6 border border-[rgba(158,158,175,0.12)] flex flex-col justify-between transition-all duration-400 relative overflow-hidden hover:-translate-y-2 hover:border-accent-gold h-full ${
